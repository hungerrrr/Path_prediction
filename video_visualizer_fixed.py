...
def convert_original_to_video_coord(orig_x, orig_y):
    """预测JSON中的坐标已经是拼接后640x240画布坐标：x在RGB右半区[320,640]，y在[0,240]。
    因此可视化阶段只做最终裁剪，不再二次缩放或强制映射。
    """
    new_x = np.clip(float(orig_x), 0, TARGET_VIDEO_W - 1)
    new_y = np.clip(float(orig_y), 0, TARGET_VIDEO_H - 1)
...
def load_pred_json(json_path):
    ...
    for item in data["frames"]:
        frame_map[item["frame_index"]] = item
    return frame_map
...
def render_visual_video():
    ...
    sorted_pred_fids = sorted(frame_data_map.keys())
    while cap.isOpened():
        ret, frame = cap.read()
        ...
        if not frame_info and sorted_pred_fids:
            prev = [f for f in sorted_pred_fids if f <= current_fid]
            nearest_fid = prev[-1] if prev else sorted_pred_fids[0]
        ...
        ts = frame_info.get("timestamp_sec", round(current_fid / FPS, 2)) if frame_info else round(current_fid / FPS, 2)
