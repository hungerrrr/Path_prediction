...
STATE_SCHEMA_VERSION = "streaming_predictor_state.v1"
...
class StreamingPredictor:
    ...
    def _update_single(self, person: PersonFrameData) -> PredictionResult:
        ...
        corridor = self._corridor(paths_pixel, self._bboxes[pid])
    ...
    def _infer(
        ...
        with torch.inference_mode():
            if self.method == "gru":
                ...
                return path[None], np.array(
                    [self.gru_confidence], dtype=np.float32,
                )
