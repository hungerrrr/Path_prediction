...
def load_advanced_checkpoint(
    ...
    """仅加载方案 B 的 Leapfrog Diffusion 检查点。"""

    checkpoint = torch.load(path, map_location=device, weights_only=False)
    schema = str(checkpoint.get("schema_version", ""))
    if not schema.startswith("leapfrog_diffusion"):
        raise ValueError(f"方案 B 不支持该检查点: {schema}")
    model = LeapfrogDiffusionPredictor(**checkpoint["model_config"])
    model.load_state_dict(checkpoint["model_state"])
    model.to(device).eval()
    return model, checkpoint, "leapfrog_diffusion"
...
def run_prediction(
    ...
    default_samples = int(checkpoint.get("num_samples", 6))
