...
from path_prediction.window_predictor import (
    CENTER_POINTS,
    CENTER_WEIGHTS,
    CandidatePath,
    FrameData,
...
def _make_eigen_mock_model(pred_frames=90, num_modes=6):
    """EigenTransformer 模拟对象：__call__ 返回 (paths, logits) 元组。"""
    model = MagicMock(spec=nn.Module)
    def forward(obs):
        B = obs.shape[0]
        paths = torch.randn(B, num_modes, pred_frames, 2)
...
class TestComputeCenter:
    def test_weighted_keypoints(self):
        ...
        bbox = (100, 50, 200, 150)
        center = WindowPredictor._compute_center(kp, bbox)
...
class TestCorridor:
    def test_corridor_basic(self):
        ...
        result = WindowPredictor._corridor(paths, bbox, margin=0.0)
    ...
    def test_corridor_with_margin(self):
        ...
        result = WindowPredictor._corridor(paths, bbox, margin=10.0)
        # bbox: [100,50,150,100]
...
class TestFromCheckpoint:
    ...
    @patch("path_prediction.window_predictor.torch.load")
    def test_load_gru(self, mock_torch_load):
        ...
        predictor = WindowPredictor.from_checkpoint(
    ...
    def test_from_checkpoint_invalid_method(self, mock_torch_load):
        mock_torch_load.return_value = {}
        try:
            WindowPredictor.from_checkpoint("dummy.pt", "invalid", device="cpu")
    ...
    def test_from_checkpoint_cuda_unavailable(self):
        try:
            WindowPredictor.from_checkpoint("dummy.pt", "gru", device="cuda")
...
class TestIntegrationBoundaries:
    def test_unified_frame_contract(self):
        config = _make_config(obs_seconds=0.1, fps=10.0)
        predictor = _make_predictor(config=config)
        person = _make_person(frame_id=0)
        records = predictor.update_unified({
            "video_path": "camera/demo.mp4",
            "video_id": "demo-video",
            "frame_index": 0,
            "timestamp_sec": 0.0,
            "persons": [{
        ...
        record = records[0]
        assert record["video_id"] == "demo-video"
        assert record["frame_index"] == 0
        assert record["timestamp_sec"] == 0.0
        assert record["predicted_activity_region"]["type"] == "polygon"
        assert record["predicted_trajectory"]
        assert 0.0 <= record["path_confidence"] <= 1.0
        assert record["confidence_type"] == "mc_dropout_ade_v1"
        assert record["confidence_details"]["mc_samples"] >= 2
    ...
    def test_state_is_json_serializable(self):
        ...
        assert restored["schema_version"] == "window_predictor_state.v1"
    ...
    def test_state_method_mismatch_is_rejected(self):
        ...
        with pytest.raises(ValueError, match="状态属于"):
            predictor.set_state({
                "schema_version": "window_predictor_state.v1",
    ...
    def test_checkpoint_method_mismatch_is_rejected(self, mock_torch_load):
        ...
        with pytest.raises(ValueError, match="不匹配"):
            WindowPredictor.from_checkpoint(
                "dummy.pt",
                "gru",
                device="cpu",
            )
